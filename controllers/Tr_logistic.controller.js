...
const createTrLogisticGambar = async (req, res) => {
  try {
    ...
    if (!req.file || req.file.length === 0) {
      const newData = new Tr_logistic({ ...dynamicFields, Tr_logistic_gambar: "NO-GAMBAR" })
      await newData.save()
    } else {
      const newData = new Tr_logistic({ ...dynamicFields, Tr_logistic_gambar: req.file.filename })
      await newData.save()
    }
